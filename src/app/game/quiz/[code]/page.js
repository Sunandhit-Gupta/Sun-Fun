...
import { fireConfetti, fireSideConfetti } from "@/lib/confetti"
...
function calculateRankDelta(players, scores, prevRanks) {
  ...
  // Check if this is the very first calculation
  ...
  sorted.forEach((p, index) => {
    ...
    if (isFirstRound) {
      // ✅ FIX: Force "same" (dash) for the first question
      delta[p.id] = "same"
    }
...
export default function QuizGamePage() {
  ...
  useEffect(() => {
    ...
    socket.on("reveal-answer", ({ correctAnswer, scores, stats, answers }) => {
      ...
      const prevRanks = rankHistoryRef.current

      const { newRanks, delta } = calculateRankDelta(
        currentPlayers,
        scores,
        prevRanks
      )
